...
// POST /api/agents/add_transfer_phonenumber_platform/[agentId] - Add transfer phone number platform for agent
export async function POST(
  ...
  try {
    ...
    const response = await fetch(`${backendUrl}/agents/add_transfer_phonenumber_platform/${agentId}`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json({
      ...
      message: 'Transfer phone number platform added successfully'
    });
  } catch (error) {
    return NextResponse.json({ 
