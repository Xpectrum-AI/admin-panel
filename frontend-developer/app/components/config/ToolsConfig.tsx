...
const ToolsConfig = forwardRef<HTMLDivElement, ToolsConfigProps>(({ agentName = 'default', modelConfig, voiceConfig, transcriberConfig, onAgentCreated, isEditing = false, existingAgent, existingConfig }, ref) => {
  ...
  React.useEffect(() => {
    try {
      const savedConfig = localStorage.getItem('toolsConfigState');
      ...
      console.warn('Failed to load saved tools config:', error);
    }
  }, []); // Remove isEditing dependency to avoid infinite loops
