...
export default function AgentsTab({}: AgentsTabProps) {
  ...
  useEffect(() => {
    ...
    setCurrentOrganizationId(ENVIRONMENT_CONFIG.DEFAULT_ORGANIZATION_ID);
  }, []);
  ...
  }, [fetchAgents, selectedAgent?.name]);
                ...
                <div className={` ${isDarkMode ? 'bg-gradient-to-br from-gray-800/30 to-gray-900' : 'bg-gradient-to-br from-gray-50/30 to-white'} max-w-4xl mx-auto lg:max-w-none overflow-y-auto`}>
                  {activeConfigTab === 'model' && (
                    <ModelConfig 
                      ...
                      existingConfig={selectedAgent ? {
                        firstMessage: selectedAgent.initial_message || '',
                        systemPrompt: selectedAgent.initial_message || '',
                        selectedModelProvider: selectedAgent.provider || 'OpenAI',
                        selectedModel: selectedAgent.model || 'GPT-4o'
                      } : null}
                      isEditing={isEditing}
                    />
                  )}
                  ...
                  {activeConfigTab === 'voice' && (
                    <VoiceConfig 
                      ...
                      existingConfig={selectedAgent?.tts_config || null}
                      isEditing={isEditing}
                    />
                  )}
                  ...
                  {activeConfigTab === 'transcriber' && (
                    <TranscriberConfig 
                      ...
                      agentName={selectedAgent?.name || 'default'}
                      onConfigChange={setTranscriberConfig}
                      existingConfig={selectedAgent?.stt_config || null}
                      isEditing={isEditing}
                    />
                  )}
