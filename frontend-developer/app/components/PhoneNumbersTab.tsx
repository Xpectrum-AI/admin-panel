...
import { Phone, Plus, Search, Sparkles, Activity, Zap } from 'lucide-react';
        ...
        <div className={`p-8 border-b rounded-t-2xl ${isDarkMode ? 'border-gray-700/50 bg-gradient-to-r from-blue-900/20 to-indigo-900/20' : 'border-gray-200/50 bg-gradient-to-r from-blue-50 to-indigo-50'}`}>
          <div className="flex items-center justify-between">
            <div className="space-y-2">
              ...
              <p className={`text-lg ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Manage your communication channels</p>
            </div>
            <button className="group relative px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl flex items-center gap-3">
                 ...
                 {samplePhoneNumbers.map((phoneNumber) => (
                   <button
                     ...
                     className={`w-full p-4 rounded-xl text-left transition-all duration-300 transform hover:scale-[1.02] ${
                       selectedNumber?.id === phoneNumber.id
                         ? isDarkMode 
                     ...
                     <div className="flex items-start gap-4">
                       ...
                       <div className="flex-1 min-w-0">
                         <div className="flex items-center justify-between mb-1">
                           <h3 className={`font-semibold truncate ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{phoneNumber.number}</h3>
                         ...
                         <p className={`text-sm mb-1 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>{phoneNumber.description}</p>
             ...
             <div className="flex-1">
               {selectedNumber ? (
                   ...
                   <div className="mb-8">
                     <div className="flex items-center gap-4 mb-4">
                       ...
                       <div>
                         ...
                         <p className={isDarkMode ? 'text-gray-300' : 'text-gray-600'}>{selectedNumber.number}</p>
                         <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>{selectedNumber.description}</p>
                       </div>
                     </div>
                     ...
                     <div className="flex items-center gap-2">
                       <div className={`w-2 h-2 rounded-full ${
                         selectedNumber.status === 'active' ? 'bg-green-500 animate-pulse' :
                         selectedNumber.status === 'inactive' ? 'bg-red-500' :
                         'bg-yellow-500'
                       }`}></div>
                       <span className={`text-sm font-medium capitalize ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{selectedNumber.status}</span>
                  ...
                  {activeTab === 'inbound' ? (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      <div className="space-y-4">
                         ...
                         <div>
                           <label className={`block text-sm font-semibold mb-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Greeting Message</label>
                           <textarea
                             rows={3}
                         ...
                         <div>
                           <label className={`block text-sm font-semibold mb-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Call Routing</label>
                           <select className={`w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 backdrop-blur-sm transition-all duration-300 ${isDarkMode ? 'border-gray-600 bg-gray-800/80 text-gray-200' : 'border-gray-200 bg-white/80 text-gray-900'}`}>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      <div className="space-y-4">
                        ...
                        <div>
                          <label className={`block text-sm font-semibold mb-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Caller ID</label>
                          <input
                            type="text"
                        ...
                        <div>
                          <label className={`block text-sm font-semibold mb-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Dialing Strategy</label>
                          <select className={`w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500 backdrop-blur-sm transition-all duration-300 ${isDarkMode ? 'border-gray-600 bg-gray-800/80 text-gray-200' : 'border-gray-200 bg-white/80 text-gray-900'}`}>
                      ...
                      <div className="space-y-4">
                        <div>
                          <label className={`block text-sm font-semibold mb-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Retry Settings</label>
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                 ...
                 <div className="text-center py-12">
                   ...
                   <h3 className={`text-xl font-semibold mb-2 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Select a Phone Number</h3>
                   <p className={isDarkMode ? 'text-gray-300' : 'text-gray-600'}>Choose a phone number from the sidebar to configure its settings</p>
                 </div>
               )}
            </div>
          </div>
