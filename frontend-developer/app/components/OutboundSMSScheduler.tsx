...
export default function OutboundSMSScheduler({ refreshTrigger }: OutboundSMSSchedulerProps) {
  ...
  const validateForm = (): boolean => {
    ...
    if (!schedulerForm.receiving_number.trim()) {
      errors.receiving_number = 'From number is required';
    } else if (!/^\+[1-9]\d{1,14}$/.test(schedulerForm.receiving_number)) {
      errors.receiving_number = 'Please enter a valid phone number (e.g., +1234567890)';
    }
    
    // To number validation
    if (!schedulerForm.recipient_phone.trim()) {
      errors.recipient_phone = 'To number is required';
  ...
  const handleSendSMSMessage = async () => {
    ...
    try {
      ...
      const result = await SMSService.sendMessage(
        schedulerForm.receiving_number || '', // from_number
        schedulerForm.recipient_phone, // to_number
          ...
          <div className={`rounded-xl p-6 max-w-2xl w-full shadow-2xl min-h-[500px] flex flex-col justify-center ${isDarkMode ? 'bg-gray-800/95 backdrop-blur-md' : 'bg-white/95 backdrop-blur-md'}`} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-4 flex justify-center">
              <button
                ...
                disabled={sending || !schedulerForm.recipient_phone || !schedulerForm.message_text || !schedulerForm.receiving_number}
