...
export default function OutboundWhatsappScheduler({ refreshTrigger }: OutboundWhatsappSchedulerProps) {
  ...
  const loadScheduledEvents = useCallback(async () => {
    ...
    try {
      // TODO: Implement WhatsApp scheduled events API call
      // For now, we'll use empty array as placeholder
      setScheduledEvents([]);
    } catch (err: unknown) {
      const errorMessage = err instanceof Error ? err.message : 'Unknown error';
      console.error('Error loading scheduled WhatsApp events:', errorMessage);
      setScheduledEvents([]);
    } finally {
  ...
  const handleDeleteScheduledEvent = async (eventId: string) => {
    if (!confirm('Are you sure you want to delete this scheduled WhatsApp event?')) {
      return;
    }

    try {
      // TODO: Implement WhatsApp scheduled event deletion API call
      setSchedulerSuccess('Scheduled WhatsApp event deleted successfully!');
      ...
      const errorMessage = err instanceof Error ? err.message : 'Unknown error';
      setSchedulerError('Failed to delete scheduled WhatsApp event: ' + errorMessage);
    }
  };

  // Outbound scheduler functions
  const handleScheduleOutboundWhatsApp = async () => {
    ...
    try {
      // TODO: Implement WhatsApp scheduling API call
      setSchedulerSuccess('Outbound WhatsApp message scheduled successfully!');
      ...
      const errorMessage = err instanceof Error ? err.message : 'Unknown error';
      setSchedulerError('Failed to schedule outbound WhatsApp: ' + errorMessage);
      console.error('Error scheduling outbound WhatsApp:', err);
    } finally {
      setScheduling(false);
    }
  };
      ...
      <div className="w-80 border-r border-gray-200/50 flex flex-col">
        <div className="p-4 border-b border-gray-200/50">
          ...
          <div className="relative group">
            ...
            <input
              type="text"
              placeholder="Search scheduled WhatsApp events..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              aria-label="Search scheduled WhatsApp events"
          ...
          {loadingScheduledEvents ? (
            <div className="flex items-center justify-center py-8">
              ...
              <span className={`ml-2 text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                Loading scheduled WhatsApp events...
              </span>
            </div>
          ) : filteredScheduledEvents.length === 0 ? (
            <div className="text-center py-8">
              <WhatsAppIcon className={`h-8 w-8 mx-auto mb-3 ${isDarkMode ? 'text-gray-600' : 'text-gray-400'}`} />
              <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                {searchTerm ? 'No WhatsApp events match your search' : 'No scheduled WhatsApp events found'}
              </p>
            </div>
          ) : (
        ...
        {selectedScheduledEvent ? (
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                  Scheduled WhatsApp Event Details
                </p>
              </div>
            </div>
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              <WhatsAppIcon className={`h-16 w-16 mx-auto mb-4 ${isDarkMode ? 'text-gray-600' : 'text-gray-400'}`} />
              <h3 className={`text-xl font-medium mb-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                Select a Scheduled WhatsApp Event
              </h3>
              <p className={`text-sm ${isDarkMode ? 'text-gray-500' : 'text-gray-500'}`}>
                Choose a scheduled WhatsApp event from the sidebar to view its details
              </p>
            </div>
          </div>
        )}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4" onClick={() => setShowCreateSchedulerModal(false)}>
          <div className={`rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 max-w-4xl w-full shadow-2xl max-h-[90vh] overflow-y-auto ${isDarkMode ? 'bg-gray-800/95 backdrop-blur-md' : 'bg-white/95 backdrop-blur-md'}`} onClick={(e) => e.stopPropagation()}>
            <h3 className={`text-lg sm:text-xl lg:text-2xl font-bold mb-4 sm:mb-6 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
              Create Scheduled WhatsApp Event
            </h3>
            ...
            <div className="mt-6 sm:mt-8 flex justify-center sticky bottom-0 bg-inherit pt-4">
              <button
                onClick={handleScheduleOutboundWhatsApp}
              ...
              >
                ...
                <span className="text-sm sm:text-base font-semibold">
                  {scheduling ? 'Creating...' : 'Schedule WhatsApp Event'}
                </span>
              </button>
            </div>
          </div>
            ...
            <div className="mt-6 sm:mt-8 flex justify-center sticky bottom-0 bg-inherit pt-4">
              <button
                onClick={async () => {
                  ...
                  try {
                    // TODO: Implement WhatsApp scheduled event update API call
                    setSchedulerSuccess('Scheduled WhatsApp event updated successfully!');
                    ...
                    const errorMessage = err instanceof Error ? err.message : 'Unknown error';
                    setSchedulerError('Failed to update scheduled WhatsApp event: ' + errorMessage);
                  } finally {
                    setScheduling(false);
                  }
                }}
              ...
              >
                ...
                <span className="text-sm sm:text-base font-semibold">
                  {scheduling ? 'Updating...' : 'Update WhatsApp Event'}
                </span>
              </button>
            </div>
          </div>
